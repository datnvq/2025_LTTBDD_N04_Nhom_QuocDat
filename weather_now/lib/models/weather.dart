...
final dt = json['dt'] as int? ?? (DateTime.now().millisecondsSinceEpoch ~/ 1000);
...
tempC: ((main['temp'] as num?)?.toDouble() ?? 0.0) - 273.15, // Kelvin -> C
description: (w0['description'] ?? '') as String,
icon: (w0['icon'] ?? '01d') as String,
humidity: (main['humidity'] as num?)?.toInt() ?? 0,
windMs: ((wind['speed'] as num?)?.toDouble() ?? 0.0),
time: DateTime.fromMillisecondsSinceEpoch(dt * 1000, isUtc: true).toLocal(),
);
}
}