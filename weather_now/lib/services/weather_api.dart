...
final uri = Uri.parse('$_base/weather?q=$cityName&appid=$apiKey&lang=$lang');
...
Future<Weather> fetchByLocation(double lat, double lon, {String lang = 'vi'}) async {
final uri = Uri.parse('$_base/weather?lat=$lat&lon=$lon&appid=$apiKey&lang=$lang');
final res = await http.get(uri);
if (res.statusCode != 200) {
throw Exception('Lỗi API: ${res.statusCode} ${res.body}');
}
